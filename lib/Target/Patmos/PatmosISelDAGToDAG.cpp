...
namespace {
  struct PatmosISelAddressMode {
    ...
    void dump() {
      errs() << "PatmosISelAddressMode " << this << '\n';
...
#if 0
    bool MatchAddress(SDValue N, PatmosISelAddressMode &AM);
    bool MatchWrapper(SDValue N, PatmosISelAddressMode &AM);
    bool MatchAddressBase(SDValue N, PatmosISelAddressMode &AM);
...
#if 0
/// MatchWrapper - Try to match PatmosISD::Wrapper node into an addressing mode.
/// These wrap things that will resolve down into a symbol reference.  If no
/// match is possible, this returns true, otherwise it returns false.
bool PatmosDAGToDAGISel::MatchWrapper(SDValue N, PatmosISelAddressMode &AM) {
...
bool PatmosDAGToDAGISel::MatchAddressBase(SDValue N, PatmosISelAddressMode &AM) {
  // Is the base register already occupied?
  if (AM.BaseType != PatmosISelAddressMode::RegBase || AM.Base.Reg.getNode()) {
    // If so, we cannot select it.
    return true;
  }

  // Default, generate it as a register.
  AM.BaseType = PatmosISelAddressMode::RegBase;
  AM.Base.Reg = N;
  return false;
}

bool PatmosDAGToDAGISel::MatchAddress(SDValue N, PatmosISelAddressMode &AM) {
  ...
  case PatmosISD::Wrapper:
    if (!MatchWrapper(N, AM))
      return false;
    break;

  case ISD::FrameIndex:
    if (AM.BaseType == PatmosISelAddressMode::RegBase
        && AM.Base.Reg.getNode() == 0) {
      AM.BaseType = PatmosISelAddressMode::FrameIndexBase;
  ...
  case ISD::ADD: {
    PatmosISelAddressMode Backup = AM;
...
/// match by reference.
bool PatmosDAGToDAGISel::SelectAddr(SDValue N,
                                    SDValue &Base, SDValue &Disp) {
  PatmosISelAddressMode AM;
  ...
  Base  = (AM.BaseType == PatmosISelAddressMode::FrameIndexBase) ?
...
SDNode *PatmosDAGToDAGISel::SelectIndexedLoad(SDNode *N) {
  ...
  switch (VT.SimpleTy) {
  case MVT::i8:
    Opcode = Patmos::MOV8rm_POST;
    break;
  case MVT::i16:
    Opcode = Patmos::MOV16rm_POST;
    break;
  default:
    return NULL;
  }
...
SDNode *PatmosDAGToDAGISel::SelectIndexedBinOp(SDNode *Op,
...
SDNode *PatmosDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse())
      return CurDAG->SelectNodeTo(Node, Patmos::ADD16ri, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, MVT::i16));
    return CurDAG->getMachineNode(Patmos::ADD16ri, dl, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, MVT::i16));
  }
  case ISD::LOAD:
  ...
  case ISD::ADD:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           ...
                           Patmos::ADD8rm_POST, Patmos::ADD16rm_POST))
      return ResNode;
  ...
  case ISD::SUB:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           Patmos::SUB8rm_POST, Patmos::SUB16rm_POST))
      return ResNode;
  ...
  case ISD::AND:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           Patmos::AND8rm_POST, Patmos::AND16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                Patmos::AND8rm_POST, Patmos::AND16rm_POST))
  ...
  case ISD::OR:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           Patmos::OR8rm_POST, Patmos::OR16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                Patmos::OR8rm_POST, Patmos::OR16rm_POST))
  ...
  case ISD::XOR:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           Patmos::XOR8rm_POST, Patmos::XOR16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                Patmos::XOR8rm_POST, Patmos::XOR16rm_POST))
