...
- Generate Eclipse project files using cmake (preferably keep your build dir separate 
...
printing in ~/.gdbinit with the Standard Process Launcher, but this launcher does not 
show formatted results in the variable browser).

In the Debugger Tab, make sure the command file points to your modified gdbinit file 
(when calling gdb on the command line or using the Standard Launcher, gdb will always 
load ~/.gdbinit. When using GDB DSF ~/.gdbinit will *not* be loaded automatically).
