; RUN: llc < %s -mpatmos-basicblock-align=8 | FileCheck %s
; RUN: llc < %s -mpatmos-basicblock-align=8 -filetype=obj -o %t -mforce-block-labels;\
; RUN: patmos-ld %t -nostdlib -static -o %t --section-start .text=1;\
; RUN: llvm-objdump %t -d | FileCheck %s --check-prefix ALIGN
; RUN: LLC_ARGS="-mpatmos-basicblock-align=8"; %test_no_runtime_execution
; END.
;//////////////////////////////////////////////////////////////////////////////////////////////////
;
; Tests that we can change the basic block alignment to 8 bytes.
;
