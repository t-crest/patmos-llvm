; RUN: llc < %s -mpatmos-basicblock-align=32 | FileCheck %s
; RUN: llc < %s -mpatmos-basicblock-align=32 -filetype=obj -o %t -mforce-block-labels;\
; RUN: patmos-ld %t -nostdlib -static -o %t --section-start .text=1;\
; RUN: llvm-objdump %t -d | FileCheck %s --check-prefix ALIGN
; RUN: LLC_ARGS="-mpatmos-basicblock-align=32"; %test_no_runtime_execution
; END.
;//////////////////////////////////////////////////////////////////////////////////////////////////
;
; Tests that we can change the basic block alignment to 32 bytes.
;
...
; ALIGN: .LBB0_{{[1-2]}}
; ALIGN-NEXT: {{[0-9]*[0|2|4|6|8|a|c|e]0}}:
if.else:                                          ; preds = %entry
  %1 = sub nsw i32 %0, 7
  br label %if.end
